...
export default function DetailPengajuanDTH() {
  ...
  const getDetailDth = async () => {
    try {
      const { data } = await bprNusaServer.get(`/pengajuan-kredit/admin/data/detail-dth/${kreditId}`, {
        headers: {
      ...
      <CCol xs={12}>
        <CCard>
          <CCardHeader style={{ fontSize: 18, fontWeight: 'bold' }}>
            Detail Pengajuan Dana Talangan Haji
